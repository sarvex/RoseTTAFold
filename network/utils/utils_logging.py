...
def write_info_file(model, FLAGS, UNPARSED_ARGV, wandb_log_dir=None):
    ...
    filename_log = f"info_{time_str}.txt"
    filename_git_diff = f"git_diff_{time_str}.txt"
    ...
    with open(os.path.join(log_dir, filename_log), "w") as file:
        label = subprocess.check_output(["git", "describe", "--always"]).strip()
        file.write(f'latest git commit on this branch: {str(label)}' + '\n')
        file.write('\nFLAGS: \n')
        for key in sorted(vars(FLAGS)):
            file.write(f'{key}: {str(vars(FLAGS)[key])}' + '\n')
        ...
        for argument in sys.argv:
            bash_command += f' {argument}'
        file.write(bash_command)
    # write 'git diff' output into extra file
    subprocess.call(
        [f"git diff > {os.path.join(log_dir, filename_git_diff)}"], shell=True
    )
